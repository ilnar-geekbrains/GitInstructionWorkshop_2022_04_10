...
Ветка — это копия проекта, созданная для внесения отдельностоящих изменений. Также, обязательно существует основная ветка (master), в которую сливают изменения из других веток. Ветки позволяют работать над проектом без угрозы фатальных изменений. Можно эксперементировать, не боясь все испортить. А если получилось хорошо, слить свои изменения в основную ветку main.
